...
// ─── Constants ────────────────────────────────────────────────────────────────
const TABS = ["Overview", "Users", "Products", "Rooms", "Feedback", "Personas"];
...
export default function AdminDashboard() {
  ...
  const [userView, setUserView] = useState("table"); // "table" | "branch"

  // All users for branch view (unpaginated)
  ...
  const fetchAllUsersForBranch = useCallback(async () => {
    if (allUsersForBranch.length > 0) return; // already loaded
  ...
  const deleteUser = async (userId, name) => {
    ...
    try {
      ...
      fetchUsers(); refreshStats();
      setAllUsersForBranch([]); // force branch view refresh
    } catch (e) { showToast(e.response?.data?.message || "Failed", "error"); }
  };
            ...
            <div>
              ...
              <div className="quick-actions">
                ...
                <div className="quick-actions-row">
                  {["Users", "Products", "Rooms", "Feedback", "Personas"].map((t) => (
            ...
            <div>
              <div className="adm-section-header">
                ...
                <div className="view-toggle">
                  ...
                  <button className={`view-toggle-btn ${userView === "branch" ? "active" : ""}`} onClick={() => setUserView("branch")}>
                    By Branch
                  </button>
                </div>
                ...
                <>
                  ...
                  {branchCourses.length > 1 && (
                    <div className="branch-filter-strip">
                      ...
                      {branchCourses.map(c => (
                        <button key={c}
                          className={`branch-course-pill ${branchCourseFilter === c ? "active" : ""}`}
                          onClick={() => setBranchCourseFilter(c)}>
                          {c}
                        </button>
                      ))}
                    </div>
                  ...
                  {branchLoading ? (
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      {[...Array(4)].map((_, i) => (
                        <div key={i} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r-xl)", padding: 16, animation: `fade-in 0.25s ease ${i * 50}ms both` }}>
            ...
            <div>
              <div className="adm-section-header">
                ...
                <input type="text" placeholder="Search products…" value={productSearch}
                  ...
                  className="adm-search" />
                <button onClick={() => setShowProductModal(true)} className="adm-btn-primary" style={{ height: 36, fontSize: 12 }}>
                  + Add Product
                </button>
              </div>
                      ...
                      {products.map((p) => (
                        <tr key={p._id} className="adm-tr">
                          <td className="adm-td">
                            <div className="user-cell">
                              {p.image
                                ? <img src={p.image} alt={p.title} className="prod-img" onError={(e) => e.target.style.display = "none"} />
                          ...
                          <td className="adm-td">
                            <p style={{ fontSize: 13, fontWeight: 500 }}>{p.sellerId?.name || p.sellerName || "Admin"}</p>
                            {p.sellerId?.email && <p style={{ fontSize: 11, color: "var(--text-3)", fontFamily: "var(--mono)" }}>{p.sellerId.email}</p>}
                          </td>
                          ...
                          <td className="adm-td right">
                            <button onClick={() => deleteProduct(p._id, p.title)} className="adm-btn-sm danger">Delete</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          {activeTab === "Rooms" && (
            <div>
              <div className="adm-section-header">
                ...
                <input type="text" placeholder="Search title, location…" value={roomSearch}
                ...
                <button onClick={() => setRoomModal({})} className="adm-btn-primary" style={{ height: 36, fontSize: 12 }}>
                  + Add Room
                </button>
              </div>
              {roomLoading ? <TableSkeleton cols={7} /> : (
                <div className="adm-table-wrap">
                  <table className="adm-table">
                    <thead className="adm-thead">
                      <tr>
                        <th className="adm-th">Room</th>
                        <th className="adm-th">Type</th>
                        <th className="adm-th">Rent</th>
                      ...
                      {rooms.map((r) => (
                        <tr key={r._id} className="adm-tr">
                          <td className="adm-td">
                            <div className="user-cell">
                              {r.images?.[0]
                                ? <img src={r.images[0]} alt={r.title} className="prod-img" />
                                : <div className="prod-img-fallback">🏠</div>}
          ...
          {activeTab === "Feedback" && (
            <div>
              <div className="adm-section-header">
                ...
                <input type="text" placeholder="Search feedback…" value={feedbackSearch}
                    ...
                    <div key={fb._id} className="feedback-card" style={{ animationDelay: `${i * 40}ms` }}>
                      ...
                      <div className="feedback-actions">
                        ...
                        {["new", "in-review", "resolved", "dismissed"].map((s) => (
                          <button key={s} onClick={() => updateFeedbackStatus(fb._id, s)}
                            className={`status-chip ${fb.status === s ? "selected" : ""}`}>
                            {s}
                          </button>
                        ))}
                        <div style={{ flex: 1 }} />
          ...
          {activeTab === "Personas" && (
            <div>
              <div className="adm-section-header">
                ...
                <input type="text" placeholder="Filter persona…" value={personaFilter}
              ...
              {personaLoading ? (
                ...
                <>
                  <div className="persona-overview">
                    <div className="persona-overview-card">
                      <p className="persona-overview-num">{allUsers.filter(u => (u.skills || []).length > 0).length}</p>
                      <p className="persona-overview-label">Have Skills</p>
                    </div>
                    <div className="persona-overview-card">
                      <p className="persona-overview-num">{allUsers.filter(u => (u.interests || []).length > 0).length}</p>
                      <p className="persona-overview-label">Have Interests</p>
                    </div>
                    <div className="persona-overview-card">
                      <p className="persona-overview-num">{allUsers.filter(u => (u.goals || []).length > 0).length}</p>
                      <p className="persona-overview-label">Have Goals</p>
                    </div>
                    <div className="persona-overview-card">
                      <p className="persona-overview-num">{personas.length}</p>
                      <p className="persona-overview-label">Clusters</p>
                    </div>
                    <div className="persona-overview-card">
                      <p className="persona-overview-num">{filteredPersonas[0]?.count || 0}</p>
                      <p className="persona-overview-label">Largest Group</p>
                    </div>
                  </div>
