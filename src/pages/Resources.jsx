...
/* ─── Upload Modal ──────────────────────────────────────────────────────── */
function UploadModal({ activeTab, onClose, onSuccess, showToast, extraSubjects, onNewSubject }) {
  const [saving,      setSaving]      = useState(false);
  const [subject,     setSubject]     = useState("");   // selected subject
  const [isOther,     setIsOther]     = useState(false);
  const [customSubj,  setCustomSubj]  = useState("");
  const [unit,        setUnit]        = useState(1);    // module number counter
  const [topic,       setTopic]       = useState("");   // optional topic text
  ...
  const handleSave = async (e) => {
    ...
    // Save new custom subject globally
    if (isOther && customSubj.trim()) {
      onNewSubject(customSubj.trim());
    }

    // Build unit string: "Module 2" or "Module 2 — Topic name"
          ...
          <form onSubmit={handleSave} style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            <div>
              ...
              {isOther && (
                <div style={{ marginTop:8 }}>
                  <input
                    style={{ ...inp, borderColor: customSubj.trim() ? "#1a1a18" : "#e5e3dc" }}
                  ...
                  <div style={{ fontSize:10, color:"#9b9890", marginTop:3 }}>
                    This will be added to the subject list
                  </div>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:0, background:"#f5f4f0", borderRadius:10, padding:"3px", width:"fit-content" }}>
                <button type="button"
                  onClick={() => setUnit(u => Math.max(1, u - 1))}
                  disabled={unit <= 1}
            ...
            <div>
              ...
              <div style={{ position:"relative" }}>
                <span style={{ position:"absolute", left:11, top:"50%", transform:"translateY(-50%)", fontSize:13, pointerEvents:"none", color:"#9b9890" }}>▶</span>
                <input style={{...inp, paddingLeft:28}} placeholder="YouTube video or playlist URL" value={lectureLink} onChange={e => setLectureLink(e.target.value)} type="url" />
              </div>
            ...
            <div>
              ...
              <div style={{ position:"relative" }}>
                <span style={{ position:"absolute", left:11, top:"50%", transform:"translateY(-50%)", fontSize:13, pointerEvents:"none", color:"#9b9890" }}>↗</span>
                <input style={{...inp, paddingLeft:28}} placeholder="Google Drive, PDF link…" value={notesLink} onChange={e => setNotesLink(e.target.value)} type="url" />
              </div>
            ...
            <div>
              ...
              <textarea style={{...inp, resize:"none", minHeight:50, fontSize:12}} placeholder="Optional — any extra context" value={description} onChange={e => setDescription(e.target.value)} rows={2} />
            </div>
...
export default function Resources() {
  ...
  // Custom subjects persisted in localStorage
  ...
  /* When a new custom subject is added via the form */
  ...
  /* All unique subjects: base + custom + any from DB not already covered */
  ...
  // Tag filter list: deduplicated across base + custom + db
