...
export default function DashboardLayout() {
  ...
  const allLinks1 = [
    ...
    // ✅ Connection Requests — only shown when logged in
    ...(user
      ? [
          {
        name: "Connections",
        path: "/connections",
        imgsrc: "/images/connections.svg",
        authType: null,
      },
        ]
      : []),
  ...
  const handleNavClick = (e, link) => {
    ...
    if (
      link.authType === "profile" &&
      (!user.branch || !user.year || !user.section)
    ) {
      e.preventDefault();
      showAuthToast(
        "⚙️ Please setup your profile first — add Branch, Year & Section",
        ...
        <aside className="hidden sm:flex flex-col w-44 shrink-0 bg-gray-100 overflow-y-auto">
          <div className="flex flex-col p-2 gap-0.5">
            {allLinks1.map((link) => (
              ...
              >
                <img
                  className={`w-5 h-5 shrink-0 ${link.name === "My Profile" ? "rounded-full object-cover" : ""}`}
                  src={link.imgsrc}
                  alt=""
                />
